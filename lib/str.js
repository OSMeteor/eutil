...
function strTxtStrReplace(drawTextStr,dReplaceArray){
    ...
    if(drawTextStr == '' || dReplaceArray.length == 0) {
return drawTextStr;
} else
...
function stripScript(s){
  ...
  for (var i = 0; i < s.length; i++) {
    rs = rs + s.substr(i, 1).replace(new RegExp('[`~!@#-$^&*()=～《》～|{}\':;\',\\\\\\\\[\\].•€¥£·<>/?~！@#￥……&*（）——|{}【】‘；：”“\'。，、？%+_]'), '');
  }
  return rs;
}
