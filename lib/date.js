...
function dateAddMonth(d){
    var m = d.getMonth();
    if(m == 11){
return new Date(d.getFullYear() + 1,1,d.getDate(),d.getHours(),d.getMinutes(),d.getSeconds());
}
...
function dateSubMonth(d){
    var m = d.getMonth();
    if(m == 0){
return new Date(d.getFullYear() - 1,12,d.getDate(),d.getHours(),d.getMinutes(),d.getSeconds());
}
...
function dateDiff(d, objDate,interval) {
    ...
    if (arguments.length < 3 || objDate.constructor != Date) {
 return undefined; 
}
    switch (interval) {
        //计算秒差
...
function dateGetStandardTimeZone(d){
    if(!d) {
return ((new Date().getTime() - dateGetGMT()) / 3600000);
} else {
return ((d.getTime() - dateGetGMT()) / 3600000).toFixed(1);
}
}
